...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      // Fallback helper for local testing before real Supabase keys/users exist
      if (
        (email === 'admin@giftofchess.org' && password === 'adminpassword') ||
        (email === 'mock@example.com' && password === 'mockpassword')
      ) {
  ...
  return (
    <main className="min-h-screen bg-offwhite flex items-center justify-center p-6 relative">
      ...
      <div className="bg-offwhite border border-stone/30 rounded-lg shadow-md p-8 max-w-md w-full space-y-8 relative z-10">
        ...
        <div className="p-4 bg-stone/20 rounded-md border border-stone/30 flex items-start space-x-2.5 text-xs text-charcoal/70">
          <Key className="h-5 w-5 text-wood shrink-0 mt-0.5" />
          <div className="space-y-1">
            <p className="font-semibold text-wood">Local Testing Hint:</p>
            <p>You can bypass authentication in development using:</p>
            <p><strong>Email:</strong> admin@giftofchess.org</p>
